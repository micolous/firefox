...
use noise::{ec::sec1_ec2_key_to_der, *};
use nss_rs::ec::{ecdh_keygen, import_ec_public_key_from_spki, EcCurve};
...
fn qr_initiated() {
    ...
    let initiator_pub = sec1_ec2_key_to_der(&initiator_pub).expect("sec1_ec2_key_to_der");
    let initiator_pub =
        import_ec_public_key_from_spki(&initiator_pub).expect("import_ec_public_key_from_spki");

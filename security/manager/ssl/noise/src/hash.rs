...
pub trait Hash: Digest {
    ...
    fn hmac(key: &SymKey, data: &[u8]) -> Result<digest::Output<Self>> {
        ...
        // https://github.com/mozilla/nss-rs/issues/141
        let data_len = c_uint::try_from(data.len()).map_err(|_| Error::InvalidArgument)?;
        ...
        unsafe { p11::PK11_DigestOp(*context, data.as_ptr(), data_len) }.into_result()?;

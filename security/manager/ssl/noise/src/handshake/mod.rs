...
use crate::{
    ...
    handshake::{initiator::XPCInitiatorHandshake, responder::XPCResponder},
    Result,
};
...
struct XPCHandshakeService {}

impl XPCHandshakeService {
    xpcom_method!(initial_knpsk0_handshake_message => InitialKNpsk0HandshakeMessage(
    ...
    fn initial_knpsk0_handshake_message(
        ...
        let local_identity = Some(convert_to_keypair(private)?);

        let handshake = InitiatorHandshake::build(psk, local_identity, None)?;

        let handshake: RefPtr<XPCInitiatorHandshake> = handshake.into();
    ...
    xpcom_method!(build_knpsk0_responder => BuildKNpsk0Responder(
        aPsk: *const ThinVec<u8>,
    ...
    fn build_knpsk0_responder(
        ...
        let responder = Responder::build(psk, None, Some(peer_pub_key), initial_message)?;
        let responder: RefPtr<XPCResponder> = responder.into();
...
/// Create a [XPCHandshakeService]-based `nsICtapCableHandshakeService`.
#[no_mangle]
pub unsafe extern "C" fn ctap_cable_handshake_service_constructor(
    iid: *const xpcom::nsIID,
    result: *mut *mut xpcom::reexports::libc::c_void,
) -> nserror::nsresult {
    let channel: RefPtr<XPCHandshakeService> =
        XPCHandshakeService::allocate(InitXPCHandshakeService {});

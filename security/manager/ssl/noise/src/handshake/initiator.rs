...
impl From<Initiator> for RefPtr<XPCInitiator> {
    fn from(value: Initiator) -> Self {
        XPCInitiator::allocate(InitXPCInitiator {
            inner: Mutex::new(value),
        })
    }
}

xpcchannel_impl!(Initiator, XPCInitiator);
...
pub struct XPCInitiatorHandshake {
    inner: Mutex<Option<InitiatorHandshake>>,
}

impl XPCInitiatorHandshake {
    ...
    fn process_handshake_response(
        ...
        let initiator: RefPtr<XPCInitiator> = initiator.into();
...
impl From<InitiatorHandshake> for RefPtr<XPCInitiatorHandshake> {
    fn from(value: InitiatorHandshake) -> Self {
        XPCInitiatorHandshake::allocate(InitXPCInitiatorHandshake {

...
impl Responder {
    ...
    pub fn build(
        ...
        let mut ss = if let Some(local_identity) = &local_identity {
...
pub struct XPCResponder {
    inner: Mutex<Responder>,
}

impl From<Responder> for RefPtr<XPCResponder> {
    fn from(value: Responder) -> Self {
        XPCResponder::allocate(InitXPCResponder {
            inner: Mutex::new(value),
        })
    }
}

xpcchannel_impl!(Responder, XPCResponder);

...
//! Noise [CipherState] implementation.
...
impl CipherState {
    /// Create a new [CipherState] using an existing NSS [SymKey].
    ///
    /// This must be imported for use with AES-256 GCM
    /// (`Aead::import_key(AeadAlgorithms::Aes256Gcm, key)`).
    pub fn new_with_key(key: SymKey) -> Self {
        Self { k: Some(key), n: 0 }
    }

    /// Create a new [CipherState] using raw key bytes.
    pub fn new_with_key_bytes(key: &[u8; 32]) -> Result<Self> {
        nss_rs::init().map_err(|_| NS_ERROR_FAILURE)?;
    ...
    /// > Sets `k` = `key`. Sets `n` = `0`.
    ///
    /// This takes a `key` as an NSS [SymKey]. This must be imported for use with AES-256 GCM
    /// (`Aead::import_key(AeadAlgorithms::Aes256Gcm, key)`).

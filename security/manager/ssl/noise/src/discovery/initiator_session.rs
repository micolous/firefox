...
use crate::{
    ...
    ec::ec2_pubkey_to_compressed_sec1,
    hash::Hash as _,
...
impl InitiatorSession {
    ...
    pub fn try_decrypt_eid(&mut self, encrypted_eid: &[u8; 20], suffix: Option<&[u8]>) -> bool {
        let Ok(signature) =
            Sha256::HMAC_ALGORITHM.hmac(&self.eid_key.signing_key, &encrypted_eid[..16])
        else {
            return false;
        };
    ...
    pub fn as_handshake(&self) -> Result<InitiatorHandshake> {
        ...
        InitiatorHandshake::new_qr_initiated(psk, self.local_identity.clone())
    }
}

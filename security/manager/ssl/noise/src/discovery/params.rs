...
use crate::{
    discovery::{EIDKey, SharedSecret, TunnelID},
    Result,
};
...
impl Params {
    ...
    pub fn supports_websocket_transport(&self) -> bool {
        self.supported_transports.is_empty()
            || self
                ...
                .contains(&nsICtapCableDiscoveryParams::TRANSPORT_WEBSOCKETS)
    }
    ...
    pub fn supports_l2cap_transport(&self) -> bool {
        self.supported_transports
            .contains(&nsICtapCableDiscoveryParams::TRANSPORT_L2CAP)
    }
...
impl From<Params> for RefPtr<XPCParams> {
    fn from(value: Params) -> Self {
        XPCParams::allocate(InitXPCParams {
...
pub unsafe extern "C" fn ctap_cable_discovery_params_constructor(
    ...
    let params: RefPtr<XPCParams> = match Params::new() {
        Ok(p) => p.into(),
        Err(e) => return e,
    };

...
use crate::{
    cable_discovery::{EIDKey, SharedSecret, TunnelID},
    Result,
};
...
use xpcom::{interfaces::nsICableDiscoveryParams, RefPtr};
...
impl Params {
    ...
    pub fn supports_websocket_transport(&self) -> bool {
        self.supported_transports.is_empty()
            || self
                ...
                .contains(&nsICableDiscoveryParams::TRANSPORT_WEBSOCKETS)
    }
    ...
    pub fn supports_l2cap_transport(&self) -> bool {
        self.supported_transports
            .contains(&nsICableDiscoveryParams::TRANSPORT_L2CAP)
    }
...
/// `nsICableDiscoveryParams`-compatible XPCOM wrapper for [`Params`][].
#[xpcom(implement(nsICableDiscoveryParams), atomic)]
pub struct CableDiscoveryParams {
    inner: Mutex<Params>,
}

impl CableDiscoveryParams {
...
impl From<Params> for RefPtr<CableDiscoveryParams> {
    fn from(value: Params) -> Self {
        CableDiscoveryParams::allocate(InitCableDiscoveryParams {
            inner: Mutex::new(value),
        })
    }
}

/// Create a new `nsICableDiscoveryParams`-compatible [`Params`][]
#[no_mangle]
pub unsafe extern "C" fn cable_discovery_params_constructor(
    ...
    let params: RefPtr<CableDiscoveryParams> = match Params::new() {
        Ok(p) => p.into(),
        Err(e) => return e,
    };

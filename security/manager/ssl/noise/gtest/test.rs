...
pub extern "C" fn Rust_NoiseHandshakeNKpsk0() {
    ...
    let initiator_hs = InitiatorHandshake::new_state_assisted(
        &psk,
        responder_pub.as_slice().try_into().unwrap(),
    )
    .expect("initial_handshake_message");
...
pub extern "C" fn Rust_NoiseHandshakeErrors() {
    ...
    // Reset point form
    initiator_msg[0] = 4;
    let responder = Responder::new_qr_initiated(&psk, &initiator_pub, &initiator_msg)
        .expect("build_responder");

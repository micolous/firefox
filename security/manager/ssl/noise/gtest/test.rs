...
pub extern "C" fn Rust_NoiseHandshakeKNpsk0() {
    ...
    let initiator_hs = InitiatorHandshake::build(&psk, Some(initiator_identity), None)
        .expect("initial_handshake_message");

    let mut responder = Responder::build(
        &psk,
        None,
...
pub extern "C" fn Rust_NoiseHandshakeNKpsk0() {
    ...
    let initiator_hs = InitiatorHandshake::build(
        &psk,
...
pub extern "C" fn Rust_NoiseHandshakeErrors() {
    ...
    expect_eq!(true, InitiatorHandshake::build(&psk, None, None).is_err());
    let fake_response: [u8; 81] = nss_rs::random();
    expect_eq!(
        true,
        Responder::build(&psk, None, None, &fake_response).is_err()
    );
    ...
    expect_eq!(
        ...
        Responder::build(
            ...
            None,
            Some(&initiator_pub),
            &initiator_hs.initial_message()[..64],
        )
        .is_err()
    );
    ...
    expect_eq!(
        true,
        Responder::build(&psk, None, Some(&initiator_pub), &initiator_msg).is_err()
    );

    // Reset point form
    initiator_msg[0] = 4;
    let responder = Responder::build(&psk, None, Some(&initiator_pub), &initiator_msg)
        .expect("build_responder");

...
let gCableDiscoveryService = Cc[
  "@mozilla.org/security/cablediscoveryservice;1"
].createInstance(Ci.nsICableDiscoveryService);

function newCableDiscoveryParams() {
  return Cc["@mozilla.org/security/cablediscoveryparams;1"].createInstance(
    Ci.nsICableDiscoveryParams
  );
}

add_task(async function test_cable_discovery_param_defaults() {
  let params = newCableDiscoveryParams();
  equal(
